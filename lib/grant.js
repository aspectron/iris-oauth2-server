...
console.log("this.grantType", this.grantType)
...
function useAuthCodeGrant (done) {
  ...
  this.model.getAuthCode(code, function (err, authCode) {
    if (err) return done(error('server_error', false, err));
console.log("authCode.expires", authCode.expires, self.now)
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(accessToken, this.client.clientId, expires, this.user, this.scopes, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
