...
function extractCredentials (done) {
  ...
  if (!this.client.clientId){
    return done(error('invalid_client', 'Missing client_id parameter'));
  }else if(!this.client.clientId.match(this.config.regex.clientId)) {
    return done(error('invalid_client', 'Invalid client_id parameter'));
...
function checkClient (done) {
  var self = this;
  self.model.getClient({
    req: self.req,
    clientId: self.client.clientId,
    clientSecret: self.client.clientSecret
  }, function (err, client) {
...
function useAuthCodeGrant (done) {
  ...
  var self = this;
  self.model.getAuthCode({req: self.req, authCode: code}, function (err, authCode) {
    if (err)
    ...
    if (!authCode.user && !authCode.userId) {
      return done(error('server_error', false, 'No user/userId parameter returned from getauthCode'));
    }

    self.scopes = authCode.scopes;
    if (!self.scopes || !self.scopes.length) {
      return done(error('server_error', false, 'No scope parameter returned from getauthCode'));
    }
...
function usePasswordGrant (done) {
  ...
  var self = this;
  return self.model.getUser({req: self.req, username: uname, password: pword}, function (err, user) {
    if (err)
      return done(error('server_error', false, err));
    if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  var self = this;
  this.model.getRefreshToken({req: self.req, token: token}, function (err, refreshToken) {
    if (err) return done(error('server_error', false, err));
    ...
    if (self.model.revokeRefreshToken) {
      return self.model.revokeRefreshToken({req: self.req, token: token}, function (err) {
        if (err) return done(error('server_error', false, err));
        done();
      });
    }
...
function useClientCredentialsGrant (done) {
  ...
  var self = this;
  return self.model.getUserFromClient({req: self.req, clientId: clientId, clientSecret: clientSecret}, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useExtendedGrant (done) {
  var self = this;
  self.model.extendedGrant({req: self.req,  grantType: self.grantType}, function (err, supported, user) {
    if (err) {
...
function checkGrantTypeAllowed (done) {
  var self = this;
  self.model.grantTypeAllowed({req: self.req, clientId: self.client.clientId, grantType: self.grantType}, function (err, allowed) {
    if (err) return done(error('server_error', false, err));

    if (!allowed) {
      return done(error('invalid_client', 'The grant type is unauthorised for this client_id'));
    }
