...
/*
OAuth2Server.AuthCodeGrant = AuthCodeGrant;
OAuth2Server.Grant = Grant;
OAuth2Server.Authorise = Authorise;
OAuth2Server.error = error;
*/
/**
 * Constructor
 *
...
function OAuth2Server (config) {
	...
	this.useRedirection = !config.skipRedirection;
 ...
 */
OAuth2Server.prototype.authorise = function (scopes) {
	var self = this;

	return function (req, res, next) {
		return new Authorise(self, req, scopes, next);
	};
};
 ...
 * @return {Function}       middleware
 */
OAuth2Server.prototype.authCodeGrant = function (check, callback) {
	var self = this;

	return function (req, res, next) {
		new AuthCodeGrant(self, req, res, next, check, callback);
	};
};
